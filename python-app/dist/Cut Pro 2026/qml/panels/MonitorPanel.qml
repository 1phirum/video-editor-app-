...
Rectangle {
    ...
    readonly property bool deferActiveSource:
        Boolean(activeMedia && activeMedia.deferMonitorLoad === true)
        || Boolean(activeClip && activeClip.timelineRenderMode === "lightweight")
    readonly property url playerSourceUrl:
        !deferActiveSource || deferredSourceArmed ? sourceUrl : ""
    ...
    function seekPlayer() {
        if (!activeClip || !activeMedia || activeMedia.kind === "image"
                || playerSourceUrl.toString() === "")
            return
        var localPosition = Backend.playheadMs - activeClip.startMs + activeClip.sourceInMs
        if (Math.abs(player.position - localPosition) > 100)
            player.position = Math.max(0, localPosition)
    }
    ...
    onPlayingChanged: {
        if (playing) {
            if (root.deferActiveSource && !root.deferredSourceArmed)
                root.deferredSourceArmed = true
    ...
    onActiveClipChanged: {
        root.deferredSourceArmed = !root.deferActiveSource
        if (root.deferActiveSource)
            player.stop()
        if (root.playing) {
            root.deferredSourceArmed = true
            root.startReplacementAudio()
                ...
                Item {
                    ...
                    Item {
                        ...
                        Image {
                            anchors.fill: parent
                            anchors.margins: 8
                            source: root.deferActiveSource && !root.deferredSourceArmed
                                    && root.activeMedia
                                    ? (root.activeMedia.thumbnailUrl || "") : ""
                            fillMode: Image.PreserveAspectFit
                            visible: root.deferActiveSource && !root.deferredSourceArmed
                            asynchronous: true
                        }
                        ...
                        VideoOutput {
                            ...
                            visible: root.activeMedia && root.activeMedia.kind === "video"
                                     && (!root.deferActiveSource
                                         || root.deferredSourceArmed)
                        }
